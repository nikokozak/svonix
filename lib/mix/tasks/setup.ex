defmodule Mix.Tasks.Svonix.Setup do
  ...
  @spec insert_after(String.t, Regex.t, String.t) :: {:insert_ok, String.t} | {:insert_error, String.t}
  ...
  defp inform_status({:insert_error, source}) do
    Mix.shell().info([:red, "Could not insert watcher, please do so yourself ", :reset, @config_file])
    source
  end
  defp inform_status({:insert_ok, source}) do
    Mix.shell().info([:green, "Inserted watcher in config ", :reset, @config_file])
    source
  end
  defp inform_status({:comment_ok, source}) do
    Mix.shell().info([:green, "Commented out esbuild watcher ", :reset, @config_file])
    source
  end
  defp inform_status({:comment_error, source}) do
    Mix.shell().info([:yellow, "Could not comment out esbuild watcher ", :reset, @config_file])
    source
  end
end
